...
<title>FindIt - Browse Listings (Azam Campus)</title>
...
<main class="flex-grow max-w-7xl mx-auto w-full px-4 sm:px-8 py-12 flex flex-col md:flex-row gap-12">
    ...
    <aside class="w-full md:w-64 flex-shrink-0 space-y-8">
        <form action="browse.php" method="GET" id="filterForm">
            ...
            <!-- Type Toggle -->
            <div class="mb-8">
                <h3 class="font-headline font-semibold text-lg mb-4 text-primary">Item Type</h3>
            ...
            <!-- Category Filters -->
            <div class="mb-8">
                <h3 class="font-headline font-semibold text-lg mb-4 text-primary">Categories</h3>
                <div class="space-y-3">
                    <?php 
                    $categories = [
                        'Electronics', 'Wallets & IDs', 'Keys', 'Bags', 'Clothing', 'Jewelry', 'Books & Documents', 'Other'
                    ];
            ...
            <a href="browse.php" class="block w-full text-center py-3 bg-surface-container-low text-primary font-semibold rounded-DEFAULT hover:bg-surface-container transition-colors text-sm shadow-sm">Reset Filters</a>
        </form>
    </aside>
    ...
    <section class="flex-grow flex flex-col min-w-0">
        ...
        <?php else: ?>
            ...
            <?php 
                ...
                $themeColorClass = $isLost ? 'text-[#F4A261]' : 'text-[#0F7173]';
                $themeLineClass = $isLost ? 'border-l-[#F4A261]' : 'border-l-[#0F7173]';
                
                $imagePath = !empty($item['primary_image']) ? ltrim($item['primary_image'], '/') : '';
            ?>
            <article class="bg-surface-container-lowest rounded-xl shadow-[0_4px_24px_rgba(13,27,42,0.04)] overflow-hidden border-l-4 <?= $themeLineClass ?> flex flex-col hover:-translate-y-1 transition-transform duration-300 relative group cursor-pointer" onclick="window.location.href='item-detail.php?id=<?= $item['item_id'] ?>'">
                <div class="h-48 w-full bg-surface-container-low relative overflow-hidden flex items-center justify-center">
                    ...
                    <?php else: ?>
                        <span class="material-symbols-outlined text-[48px] text-outline opacity-50">
                ...
                <div class="p-6 flex-grow flex flex-col">
                    ...
                    <div class="mt-auto pt-4 flex items-center justify-between border-t border-surface-container">
                        ...
                        <a href="item-detail.php?id=<?= $item['item_id'] ?>" class="text-sm font-semibold <?= $themeColorClass ?> hover:underline underline-offset-4" style="display:inline-block;text-align:center;">Details</a>
                    </div>
                </div>
            </article>
            <?php endforeach; ?>
